...
public class BloodDB {
	...
	{
		 String address = "https://radiant-journey-86060.herokuapp.com/mi/login"; // 서버주소
	...
	public String getRowData(String id)//처음 rowdata가져오기
	{
		 String address = "https://radiant-journey-86060.herokuapp.com/mi/"+id; // 서버주소
		    StringBuffer buffer = null;
		    try{
		        //연결, 송신
	//	        String parameter = "?"; //파라미터, 변수1=값1
	...
	public String updateValid(String number)//유효성 변경
	{
		 String address = "https://radiant-journey-86060.herokuapp.com/bdc/"+number+"/update"; // 서버주소
		    StringBuffer buffer = null;
		    try{
		        //연결, 송신
		    	String parameter = "?"; //파라미터, 변수1=값1
		        String value = "valid=" + URLEncoder.encode("false","utf-8"); // 값 넣을때는 URLEncoder로 인코딩해서 값 넣어줘야합니다.  
		        parameter += value; // 파라미터는 ?변수1=값1&변수2=값2... 이렇게 연결되고요, &로 파라미터(변수) 구분됩니다.
		        address += parameter; //실질적으로 서버주소에 파라미터값이 더해져서 요청하는 url이 완성됩니다. 주석된 부분은 수정하신다고 보시면 되요

		        URL url = new URL(address);
		        HttpURLConnection connection = (HttpURLConnection)url.openConnection();
		        connection.setRequestMethod("PUT"); // POST, GET, PUT 중 택
